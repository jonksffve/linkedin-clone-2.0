	...
	try {
		...
		if (axios.isAxiosError(error)) {
			if (error.response) {
				...
				toast.error(`An unexpected error ocurred: ${error.message}`, toastConfig);
			}
		}
	} finally {
		setIsLoading(false);
...
export const createTokenAuthAPI = async (data: LoginFormInputs, setIsLoading: (value: boolean) => void) => {
	try {
		setIsLoading(true);
