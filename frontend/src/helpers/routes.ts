//FRONTEND ROUTES
export const ROUTE_INDEX = '/';
export const ROUTE_CHAT = '/chat/';
export const ROUTE_JOBS = '/jobs/';
export const ROUTE_FEED = '/feed/';
export const ROUTE_PROFILE = '/profile/';
export const ROUTE_PROFILE_EDIT = '/profile/edit/';
export const ROUTE_CONNECTIONS = '/connections/';
export const ROUTE_NOTIFICATIONS = '/notifications/';

//BACKEND ENDPOINT ROUTES
export const ENDPOINT_ACCOUNT = 'http://localhost:8000/api/account/';
export const ENDPOINT_PROFILE = 'http://localhost:8000/api/account/profile/';
export const ENDPOINT_PROFILE_EDIT = 'http://localhost:8000/api/account/profile/edit/';
export const ENDPOINT_LOGIN = 'http://localhost:8000/api/login/';
export const ENDPOINT_LOGOUT = 'http://localhost:8000/api/auth/logout/';
export const ENDPOINT_POST = 'http://localhost:8000/api/post/';
export const ENDPOINT_POST_LIKE = 'http://localhost:8000/api/post/like/';
export const ENDPOINT_COMMENT = 'http://localhost:8000/api/comment/';
export const ENDPOINT_COMMENT_LIKE = 'http://localhost:8000/api/comment/like/';
